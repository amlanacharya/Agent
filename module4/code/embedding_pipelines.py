...
try:
    ...
    logging.info("tqdm not available. Install with 'pip install tqdm' for progress bars.")
...
class EmbeddingPipeline:
    ...
    def embed_documents(
        ...
        if documents_to_embed:
            ...
            total_batches = (len(documents_to_embed) - 1) // self.batch_size + 1

            # Create batch iterator with progress bar if available
            if show_progress and TQDM_AVAILABLE:
                batch_range = tqdm(
                    range(0, len(documents_to_embed), self.batch_size),
                    total=total_batches,
                    desc="Generating embeddings"
                )
            else:
                batch_range = range(0, len(documents_to_embed), self.batch_size)
                if show_progress:
                    logger.info(f"Generating embeddings for {len(documents_to_embed)} documents in {total_batches} batches")
            ...
            for i in batch_range:
                ...
                # Show progress if tqdm not available
                ...
                # Show batch processing time if tqdm not available
                if show_progress and not TQDM_AVAILABLE:
                    elapsed = time.time() - start_time
                    logger.info(f"Batch processed in {elapsed:.2f}s ({len(batch)/elapsed:.1f} docs/sec)")
