...
def exercise4_hyde(query: str, vectorstore: Any, llm: Any, embedding_model: Embeddings) -> List[Document]:
    ...
    try:
        ...
        hypothetical_doc = _generate_hypothetical_document(query, llm)
...
def _generate_hypothetical_document(query: str, llm: Any) -> str:
...
def exercise5_step_back_prompting(query: str, retriever: BaseRetriever, llm: Any) -> List[Document]:
    ...
    try:
        ...
        general_question = _generate_general_question(query, llm)
...
def _generate_general_question(query: str, llm: Any) -> str:
...
def exercise7_lcel_query_transformation(
    ...
    # TODO: Implement LCEL query transformation chain
    ...
    branch_chain = RunnableBranch(
        (lambda x: route_query(x["query"]) == "expansion", expansion_chain),
        (lambda x: route_query(x["query"]) == "reformulation", reformulation_chain),
        (lambda x: route_query(x["query"]) == "multi_query", multi_query_chain),
        (lambda x: route_query(x["query"]) == "hyde", hyde_chain),
        (lambda x: route_query(x["query"]) == "step_back", step_back_chain),
